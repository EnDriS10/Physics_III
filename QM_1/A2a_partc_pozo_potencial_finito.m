...
%% Particula en un Pozo de Potencial Finito
% 
%  Entrega: César Endris Acosta Martínez— Problema 2
% =========================================================================
...
ep1=ep1( abs(cos(ep1)) > dx  & ((var^2) - ep1.^2 ) > 0 ); % filtrar los ceros que vienen de errores como dividir entre 0 e imaginarios
E1 = V0.*(ep1./var).^2;
...
ep2=ep2( abs(sin(ep2)) > dx  & ((var^2) - ep2.^2 ) > 0 ); % filtrar los ceros que vienen de errores como dividir entre 0 e imaginarios
E2 = V0.*(ep2./var).^2;
...
subplot(3,2,2)
hold on
plot(x_1./L, pdf1_1.*(L), 'r')
plot(x_2./L, pdf1_2.*(L), 'r')
plot(x_3./L, pdf1_3.*(L), 'r')
xline(-0.5, 'k--'); xline(0.5, 'k--');
xlim([-1.5 1.5]);
title('|\psi_1|^2 (par)')
...
subplot(3,2,4)
hold on
plot(x_1./L, pdf2_1.*(L), 'b')
plot(x_2./L, pdf2_2.*(L), 'b')
plot(x_3./L, pdf2_3.*(L), 'b')
xline(-0.5, 'k--'); xline(0.5, 'k--');
xlim([-1.5 1.5]);
title('|\psi_2|^2 (impar)')
xlabel('x/L'); ylabel('|\psi|^2 \cdot L')
...
subplot(3,2,6)
hold on
plot(x_1./L, pdf3_1.*(L), 'g')
plot(x_2./L, pdf3_2.*(L), 'g')
plot(x_3./L, pdf3_3.*(L), 'g')
xline(-0.5, 'k--'); xline(0.5, 'k--');
xlim([-1.5 1.5]);
title('|\psi_3|^2 (par)')
xlabel('x/L'); ylabel('|\psi|^2 \cdot L')
